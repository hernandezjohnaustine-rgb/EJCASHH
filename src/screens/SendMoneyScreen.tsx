...
import { ArrowLeft, Search, User, ChevronRight, Phone, Star } from "lucide-react";
    ...
    return (
      <div className="flex flex-col gap-8 h-full bg-brand-black p-6 pt-12 overflow-y-auto">
        ...
        <section className="flex flex-col items-center gap-4 py-8">
          <div className="w-20 h-20 rounded-[28px] bg-brand-blue flex items-center justify-center text-2xl font-bold border-4 border-white/5 shadow-2xl">
            {selected.initial}
          </div>
        ...
        <section className="mt-4">
           <GlassCard className="!p-4 bg-brand-navy/40">
              <div className="flex justify-between items-center">
                 ...
                 <span className="text-xs font-bold">₱85,240.50</span>
              </div>
           </GlassCard>
        </section>

        <div className="mt-auto pb-4">
          <button className="btn-primary w-full shadow-[0_0_30px_rgba(26,92,255,0.4)] h-16 text-lg font-bold">
            Send Money
          </button>
        </div>
      </div>
      ...
      <section className="flex flex-col gap-4">
        <div className="relative">
          ...
          <input 
            ...
            className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-12 pr-4 focus:outline-none focus:border-brand-neon/30 transition-all text-sm font-medium"
          />
        </div>
        
        <div className="grid grid-cols-2 gap-3">
          <button className="btn-ghost !py-3">
            <ScanLine className="w-5 h-5 text-brand-neon" />
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="flex gap-4 overflow-x-auto pb-2 -mx-6 px-6">
          {contacts.map(contact => (
            ...
            >
              <div className="w-16 h-16 rounded-[24px] bg-white/5 border border-white/10 flex items-center justify-center text-lg font-bold group-hover:bg-brand-blue/20 group-hover:border-brand-blue/30 transition-all">
                {contact.initial}
              </div>
           ...
           {contacts.map(contact => (
             ...
             >
                ...
                <div className="flex items-center gap-2">
                   {contact.saved && <Star className="w-4 h-4 text-brand-neon fill-brand-neon/20" />}
...
import { Landmark, ScanLine, PlusCircle } from "lucide-react";
