...
const services = [
  { id: "send", name: "Send", icon: ArrowUpRight, color: "#FACC15" },
  { id: "load", name: "Load", icon: Smartphone, color: "#EAB308" },
  { id: "bank", name: "Bank", icon: Landmark, color: "#CA8A04" },
  { id: "bills", name: "Bills", icon: Receipt, color: "#FDE047" },
  { id: "cards", name: "Cards", icon: CreditCard, color: "#FACC15" },
  { id: "cashin", name: "Cash In", icon: PlusCircle, color: "#EAB308" },
];

const WALLETS = (balance: number, earnings: number): Wallet[] => [
  { label: "Main Balance", balance: balance, type: "main", color: "text-white" },
  ...
  return (
    <div className="flex flex-col gap-8 pb-32">
            ...
            <div className="flex justify-between items-start">
              ...
              <div className="flex flex-col gap-2">
                <div className="w-11 h-11 bg-brand-primary/20 backdrop-blur-md rounded-2xl flex items-center justify-center border border-brand-primary/30 shadow-[0_0_15px_rgba(250,204,21,0.2)]">
                  <div className="relative text-xs font-black italic tracking-tighter text-brand-primary">UW</div>
                </div>
                <div className="flex gap-1 justify-center mt-2">
                   {wallets.map((_ , i) => (
                     <button 
                       ...
                       className={`w-1.5 h-1.5 rounded-full transition-all ${i === activeWalletIdx ? 'bg-brand-primary w-4' : 'bg-white/20'}`}
                     />
                   ))}
                </div>
              </div>
      ...
      <section className="px-6">
        ...
        <GlassCard className="!p-0 overflow-hidden bg-gradient-to-r from-brand-primary/10 to-brand-primary/5 border-white/5 group cursor-pointer active:scale-[0.98] transition-all">
          <div className="p-6 flex items-center gap-6">
            <div className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center group-hover:bg-brand-primary/20 transition-all">
          ...
          {transactions.map((tx, i) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${tx.type === 'in' ? 'bg-brand-primary/10' : 'bg-white/5 opacity-80'}`}>
