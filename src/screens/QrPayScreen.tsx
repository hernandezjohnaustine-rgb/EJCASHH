  ...
  return (
    <div className="min-h-screen bg-brand-black flex flex-col p-6 pt-12 overflow-y-auto pb-24 relative">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-64 h-64 bg-brand-blue/10 rounded-full blur-[100px]"></div>
      </div>
      ...
      <header className="flex items-center justify-between mb-8 relative z-10">
        ...
        <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10">
          <button 
            ...
            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeMode === 'scan' ? 'bg-brand-blue text-white shadow-lg' : 'text-white/40'}`}
          >
            Scan
          </button>
          <button 
            onClick={() => setActiveMode('myqr')}
            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeMode === 'myqr' ? 'bg-brand-blue text-white shadow-lg' : 'text-white/40'}`}
          >
            My QR
          </button>
        </div>
        ...
        {activeMode === 'scan' ? (
          ...
          >
            ...
            <div className="relative w-full aspect-square max-w-[320px]">
               {/* Scanner Corners */}
               <div className="absolute top-0 left-0 w-12 h-12 border-t-4 border-l-4 border-brand-neon rounded-tl-3xl z-20"></div>
               <div className="absolute top-0 right-0 w-12 h-12 border-t-4 border-r-4 border-brand-neon rounded-tr-3xl z-20"></div>
               <div className="absolute bottom-0 left-0 w-12 h-12 border-b-4 border-l-4 border-brand-neon rounded-bl-3xl z-20"></div>
               <div className="absolute bottom-0 right-0 w-12 h-12 border-b-4 border-r-4 border-brand-neon rounded-br-3xl z-20"></div>
               ...
               <div className="absolute inset-4 rounded-[40px] overflow-hidden bg-white/5 border border-white/10 flex items-center justify-center shadow-inner">
                  ...
                  <motion.div 
                    ...
                    className="absolute left-4 right-4 h-[2px] bg-brand-neon shadow-[0_0_15px_#00F0FF] z-10"
                  />
            ...
            <div className="flex gap-6">
               <button className="flex flex-col items-center gap-3 group">
                  <div className="w-16 h-16 glass-card flex items-center justify-center group-active:scale-95 transition-transform border-white/10">
                     <Smartphone className="w-6 h-6 text-brand-neon" />
                  </div>
               ...
               <button className="flex flex-col items-center gap-3 group">
                  <div className="w-16 h-16 glass-card flex items-center justify-center group-active:scale-95 transition-transform border-white/10">
                     <Scan className="w-6 h-6 text-brand-blue" />
                  </div>
          ...
          >
            ...
            <div className="relative group p-8 rounded-[48px] bg-white/[0.03] border border-white/10 shadow-2xl backdrop-blur-xl">
               <div className="absolute inset-0 bg-brand-neon/5 rounded-[48px] blur-3xl opacity-50 group-hover:opacity-100 transition-opacity"></div>
            ...
            <div className="flex flex-col items-center gap-6 w-full max-w-[320px]">
              ...
              <GlassCard className="w-full flex items-center justify-between py-4 px-6 border-white/10">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      <p className="text-[10px] text-white/40 font-bold uppercase tracking-widest">EJCASH-ID</p>
