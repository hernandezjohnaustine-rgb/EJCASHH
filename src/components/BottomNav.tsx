...
export default function BottomNav({ activeTab, setActiveTab }: BottomNavProps) {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 px-6 pb-8 pt-4 bg-brand-navy/90 backdrop-blur-xl border-t border-brand-border shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
      <div className="flex items-center justify-between max-w-lg mx-auto">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              <div className={`relative p-2 rounded-2xl transition-all ${isActive ? "text-brand-primary" : "text-brand-text/40 group-hover:text-brand-text/60"}`}>
              ...
              <span className={`text-[10px] font-medium uppercase tracking-wider ${isActive ? "text-brand-primary" : "text-brand-text/40"}`}>
                {tab.label}
              </span>
            </button>
          );
